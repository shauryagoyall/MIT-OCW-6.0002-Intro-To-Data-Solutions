...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        return str(self.src) + "->"+str(self.dest)+ " ("+str(self.get_total_distance())+", "+str(self.get_outdoor_distance())+")"
...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        if (edge.get_source() in self.nodes) and (edge.get_destination() in self.nodes):
            temp = self.edges[edge.get_source()]
            temp.append(edge)
            self.edges[edge.get_source()]=temp
        else:
